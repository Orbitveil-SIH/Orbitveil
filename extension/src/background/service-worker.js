...
// --- On-page reasoning overlay (for demo/video visibility) -----------------
// Shows the VLM's actual reasoning + chosen action directly on the live page,
// updated each step. This is what makes "the AI reasons about the screen"
// visible in a recording, since a judge watching a video can't see backend
// console logs or the extension popup. Purely visual - has no effect on the
// actual pipeline or what's sent to the server.
function __orbitveilReasoningOverlayFunc(stepNumber, reasoning, actionType, actionTarget) {
  ...
  const actionLine = actionType === "wait" || actionType === "done"
    ? actionType.toUpperCase()
    : `${actionType.toUpperCase()}${actionTarget ? " \u2192 " + actionTarget : ""}`;
...
async function clearReasoningOverlay(tab) {
  try {
    ...
    // non-fatal - tab may have navigated away or closed
  }
}
...
async function runAutomationLoop(taskDescription, onProgress = () => {}) {
  ...
  let lastTab = null; // tracked so we can clear the reasoning overlay on any exit path
  ...
  console.log(
    `[Benchmark] Server round trip: ${serverRoundTripMs.toFixed(2)} ms`
      ...
      // Show the model's actual reasoning + chosen action on the live page
      // before acting on it - this is what makes "the AI is reasoning" visible
      // in a screen recording, since a judge watching a video can't see the
      // backend console.
